...
print(f"Is CUDA present in this system? {torch.cuda.is_available()}")
...
train_data = torchvision.datasets.CIFAR10(
    root="python/dataset", train=True,
    transform=torchvision.transforms.ToTensor(), download=True)

test_data = torchvision.datasets.CIFAR10(
    root="python/dataset", train=False,
    transform=torchvision.transforms.ToTensor(), download=True)
...
# 添加tensorboard
writer = SummaryWriter("python/logs_train")
...
for i in range(epoch):
    ...
    with torch.no_grad():
        for data in test_dataloader:                        # 测试数据集中取数据
            ...
            targets = targets.to(device)
            outputs = model(imgs)
            # 这里的 loss 只是一部分数据(data) 在网络模型上的损失
    ...
    torch.save(model, "python/model_{}.pth".format(i))
    print("模型已保存")

writer.close()

# cd ~/shell;./install_common.sh
# tsocks /usr/bin/python3 /home/zad/reuse-code/python/pytorch_gpu.py
